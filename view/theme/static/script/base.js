...
// Api call for shortening long urls
function convert() {
  var spinner = '<div role="status">';
  spinner +=
    '<svg aria-hidden="true" class="h-8 text-gray-200 animate-spin dark:text-gray-600 fill-[#f4f2f1] w-full text-center" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">';
  spinner +=
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var htmltext =
        '<p id="shorturl" class="inline-block pr-4 underline text-[#0d609b] cursor-copy">' +
        shorturl +
        "</p>";
...
// Api call for registration of user
...
// Api call for signing in of existing users
...
// user logout url function
$(document).on("click", "#logout", function (e) {
  localStorage.removeItem("token");
});

// Function for menu drop down
function drop() {
...
// Menu drop down list click
document.addEventListener("click", function (event) {
  var drops = document.getElementById("drop");
  var icon = document.querySelector("#icon");
  if (drops) {
    var isClickInside =
      drops.contains(event.target) ||
      document.querySelector("#icon").contains(event.target);
...
// password validation against the rules
...
// copy button click copy function
...
// Api call for post profile details
...
$(document).ready(function () {
  $(document).on("click", ".option-btn", function (e) {
    ...
    $(".options")
      .not(options)
      .css({ transform: "translateX(50px)", opacity: 0, display: "none" });
...
$(".options").on("click", "#edit", function () {
  $(".link-edit").show();
});

$(".options").on("click", "ul #delete", function () {
  var linkId = $('input[name="link-id"]').val();
  if (confirm("Are you sure you want to delete?")) {
    $.ajax({
      type: "DELETE",
      url: "/short/api/url/" + linkId,
      contentType: false,
      processData: false,
      headers: {
        Authorization: authToken,
      ...
      success: function (response) {
        ...
        setTimeout(() => {
          $(document)
            .find("[data-id='" + linkId + "']")
            .css({ transform: "translateX(50px)", opacity: 1 });
          $(document)
            .find("[data-id='" + linkId + "']")
            .remove();
        }, 1000);
...
document.addEventListener("DOMContentLoaded", function (e) {
  var countLimit = "2024-12-31T23:59:59";
  var targetTime = new Date(countLimit);
  ...
  setInterval(() => {
    ...
    $(".time-read #countdown-day").text(days);
    $(".time-read #countdown-hrs").text(hours);
    $(".time-read #countdown-mins").text(minutes);
    $(".time-read #countdown-sec").text(seconds);
  }, 1000);
});
